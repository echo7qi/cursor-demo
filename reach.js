...
function analyzePositions(projectName) {
  if (!opsRows.length) return [];
  const daily = opsRows.filter(opsRowIsDailyRow);
  const projRows = daily.filter((r) => {
  ...
  if (!projRows.length) {
    ...
    return buildPositionStats(fuzzy);
  }
...
function buildCrossProjectPositionMatrix(wishProjectNames) {
...
function renderLatestWeekTable() {
  ...
  const merged = wishItems.map((p) => {
    ...
    const positions = analyzePositions(p.project);
  ...
  const crossInsights = buildCrossProjectPositionMatrix(merged.map((p) => p.project));
  ...
  if (crossInsights.swapSuggestions.length || crossInsights.saturationAlerts.length) {
    ...
    insightsHtml += '<div class="lwt-insights__title">🔍 跨项目综合洞察</div>';
